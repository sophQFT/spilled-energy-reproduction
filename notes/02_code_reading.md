...
CUDA 環境を作り直した（cu130 → cu128，venv 再作成）前後で，
検証スクリプトの出力が変わっていないことを確認した。

```bash
diff logs/01_alignment_check.log logs/alignment_check_20260716_0544.log
```

```bash
diff logs/02_convention_check.log logs/convention_check_20260716_0544.log
```

差分は進捗バーの速度表示（`it/s`）のみで，**数値結果は完全一致**した。

```text
2c2
< Loading weights: 100%|...| 291/291 [00:00<00:00, 11792.79it/s]
---
> Loading weights: 100%|...| 291/291 [00:00<00:00, 11817.80it/s]
```

→ 環境再構築が正しく行われたこと，および greedy 生成（`do_sample=False`）による
検証実験が決定的で再現可能であることが確認できた。
